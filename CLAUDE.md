...
**중요: "생성" 버튼 = 원본과 같은 숫자가 나오는지 별개 문제였음, 해결됨
(2026-08-10, 같은 날).** 화면 기본 폼(범용 근사)으로 생성하면 검증된
수치와 달랐다 — 엔진은 정확해도 화면이 그 정확도에 필요한 입력(80분기
실측치, 세액공제/AMT, 배당 타이밍)을 안 받고 있었기 때문. **"당진 FS
불러오기" 버튼**을 추가해서 `reference.json`을 통째로 인라인(빌드 시
`__DANGJIN_REF__`)하고, 로드 시 `test_ops.js`와 동일하게 구성한 정밀
입력을 저장해뒀다가 생성 버튼이 그걸 그대로 쓰게 했다. `test:ui2`가
프리셋 로드 후 6개 KPI를 `test_ops.js` 검증값과 직접 대조해서 전부
일치 확인 — **이제 "당진 FS 불러오기"→"생성" 순서로 누르면 실제로
원본과 같은 숫자가 나온다.** 프리셋 안 누르고 빈 폼으로 생성하면 여전히
범용 근사치가 나오는 게 맞음(범용 도구로서 정상 동작). 상세는
STATUS.md 5번 "중요 후속수정" 참조 — **다음에 UI를 건드릴 때 이 프리셋
로직(`usingPreset`/`presetInp`/`dropUnsupportedMethod3`)을 깨지 않도록
주의할 것.**
