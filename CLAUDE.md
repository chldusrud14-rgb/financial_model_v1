...
**2026-08-27에 전 시트 라이브 수식으로 전환 완료** — "입력값" 시트(노란 탭)의
노란 셀만 키인 값이고 나머지 9개 시트는 전부 그 시트를 참조하는 수식이다.
시트별 합계도 SUM() 수식. (예전 "값(baked) 기준" 서술은 무효 — 반복계산이라
수식화 불가라던 판단이 오판이었고, 실제로는 전부 직전 열 참조였다.
STATUS.md 4번/8번 참조.)
...
8. 입력값 시트의 트랜치 금액은 **약정액**(`model.con.srcs[ti+1].amount`)이다.
   ...
   상환 수식(방식 1·2·3 모두)은 약정액이 아니라 **실제 총인출액**(`amortBase` =
   그 트랜치 '인출' 행의 D열 합계, 엔진 `t.amount` 와 같음)을 기준으로 한다 — 약정액을
   곱하면 인출이 적을 때 잔액이 음수가 된다. "상환 개시 시점 잔액"도 안 된다: 거치가
   짧아 공사 중에 상환이 시작되면 일부 인출분만 나눠 갚는다(11번째 라운드).
9. 코드 주석의 "원본에 없다" 류 서술을 믿지 말고 원본 xlsm 에서 확인할 것.
   총사업비 항목이 "원본에 총액만 있다"는 주석 때문에 한참 빠져 있었다(Assum!B87:F99).
10. **엑셀 결과값은 빌더가 직접 계산해 저장한다**(`fillFormulaResults`). 수식에 새 함수를
   쓰면 그 계산기에도 추가할 것. 검증은 "저장된 결과값 ↔ 엔진 KPI"까지 볼 것 —
   극단 입력도 값으로 대조(런타임 오류만 보면 안 됨).
