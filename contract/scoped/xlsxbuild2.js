  ...
  function buildWorkbook(model, ExcelJSLib) {
    ...
    (function () {
      ...
      model.tranches.forEach(function (t, ti) {
        ...
        // 금액은 사용자가 넣은 '약정액'이어야 한다(con.srcs). model.tranches 의
        // amount 는 실제 인출액이라, 조달이 남는 경우(자본금을 늘렸거나 트랜치를
        // 넉넉히 잡은 경우) 약정액보다 작다. 그 값을 쓰면 입력값 시트가 사용자
        // 입력과 다른 숫자를 보여주고, 같은 투입순서 그룹의 비례 인출 비율도
        // 틀어져 건설이자·이자가 엔진과 어긋난다.
        put(ws, a.amount, (src.amount != null ? src.amount : t.amount) / 100, FMT_M, { fill: INPUT_FILL });
    ...
    /* =========================================================
       3. Revenue
          읽는 순서대로 계산이 흘러가게 한다:
            타임라인(운영연차·기간비중) → 발전량 → 트랙별 판매단가
            → 트랙별 매출(=발전량×비중×단가) → 영업수익(=트랙 매출 합계)
            → 가중평균 판매단가(=영업수익÷발전량)
          예전에는 영업수익 한 칸에 입력값의 트랙 비중·단가·상승률을 전부
          끌어다 곱했고, A/R 섹션에서 같은 트랙 매출을 입력값에서 또 한 번
          계산했으며, 판매단가는 매출에서 역산했다. 이제 트랙 매출은 한 번만
          만들고 영업수익과 A/R 이 그 행을 참조한다.
          입력값 참조는 마지막에 localizeInputs() 가 시트 하단 '가정' 블록으로
          모은다 — 여기서는 계산 구조만 신경 쓴다.
          실측 오버라이드 분기는 그 분기의 실제 정산단가(입력값 시트)로
          영업수익 = 발전량 × 단가 를 계산한다(값은 원본과 그대로 일치).
       ========================================================= */
    (function () {
      ...
      // --- 타임라인: 수식 속에 숨어 있던 상수(^0, *0.25)를 보이는 행으로 ---
      var yrRow = r;
      label(ws, r, '운영연차 (첫 해 = 0)', '[yr]');
      for (var n = 0; n < N; n++) {
        put(ws, pc(n) + r, periods[n].opYearIdx, '0', { noSum: true });
        ws.getCell(pc(n) + r).font = { name: FONT, size: 9, color: { argb: GRAY } };
      }
      r++;
      var fracRow = r;
      label(ws, r, '발전 기간비중 (연간 = 1)', '[배]');
      for (var n = 0; n < N; n++) {
        var p = periods[n];
        var frac = p.opMonths / 12;
        var full = ppy === 4 && p.opMonths === (12 / ppy);
        var genFrac = (inp.seasonality && full) ? inp.seasonality[p.end.getUTCMonth() + 1] : frac;
        put(ws, pc(n) + r, genFrac, '0.0000', { noSum: true });
        ws.getCell(pc(n) + r).font = { name: FONT, size: 9, color: { argb: GRAY } };
      }
      r += 2;
      ...
      for (var n = 0; n < N; n++) {
        putF(ws, pc(n) + r,
          '(' + annualGenF + ')*MAX(0,1-' + IN + IN_ADDR.degradation + '/100*' + pc(n) + yrRow + ')' +
          '*(1-' + IN + IN_ADDR.auxRate + '/100)*' + pc(n) + fracRow, '#,##0', { bold: true });
      }
      putF(ws, 'D' + r, sumFormula(r), '#,##0', { bold: true });
      r += 2;

      // --- 트랙별 판매단가 → 트랙별 매출 ---
      var trackPriceRow = {}, trackRevRow = {};
      var trackName = function (ti) { return (inp.tariffTracks[ti].name) || ('트랙' + (ti + 1)); };
      if (hasTracks) {
        IN_ADDR.tariffTrack.forEach(function (ta, ti) {
          trackPriceRow[ti] = r;
          label(ws, r, trackName(ti) + ' 판매단가', '[원/kWh]', { indent: true });
          for (var n = 0; n < N; n++) {
            if (!periods[n].isOp) { put(ws, pc(n) + r, null, '#,##0.0'); continue; }
            putF(ws, pc(n) + r, IN + ta.price + '*(1+' + IN + ta.escal + '/100)^' + pc(n) + yrRow, '#,##0.0');
          }
          r++;
        });
        IN_ADDR.tariffTrack.forEach(function (ta, ti) {
          trackRevRow[ti] = r;
          label(ws, r, trackName(ti) + ' 매출', '[KRWm]', { indent: true });
          for (var n = 0; n < N; n++) {
            if (!periods[n].isOp) { put(ws, pc(n) + r, 0, FMT_M); continue; }
            // 매출 = 발전량 × 트랙 비중 × 트랙 단가 / 1000
            putF(ws, pc(n) + r,
              pc(n) + genRow + '*' + IN + ta.share + '/100*' + pc(n) + trackPriceRow[ti] + '/1000', FMT_M);
          }
      ...
      // --- 판매단가(가중평균) · 영업수익 ---
      var priceRow = r, revRow = r + 1;
      REV_TOTAL_ROW = revRow;
      label(ws, priceRow, hasTracks ? '판매단가 (가중평균)' : '판매단가', '[원/kWh]');
      label(ws, revRow, '영업수익', '[KRWm]', { bold: true, fill: SUB_FILL });
      var firstTr = hasTracks ? trackRevRow[0] : null;
      var lastTr = hasTracks ? trackRevRow[IN_ADDR.tariffTrack.length - 1] : null;
      var anyOverride = false;
      for (var n = 0; n < N; n++) {
        var p = periods[n];
        var c = pc(n);
        if (!p.isOp) {
          put(ws, c + priceRow, null, '#,##0.0');
          put(ws, c + revRow, 0, FMT_M, { bold: true });
          continue;
        }
        if (ovrByEnd[p.endStr]) {
          anyOverride = true;
          putF(ws, c + priceRow, IN + c + IN_ADDR.ovr.price, '#,##0.0');
          putF(ws, c + revRow, c + genRow + '*' + c + priceRow + '/1000', FMT_M, { bold: true });
        } else if (hasTracks) {
          putF(ws, c + revRow, 'SUM(' + c + firstTr + ':' + c + lastTr + ')', FMT_M, { bold: true });
          putF(ws, c + priceRow, 'IF(' + c + genRow + '=0,0,' + c + revRow + '*1000/' + c + genRow + ')', '#,##0.0');
        } else {
          putF(ws, c + priceRow,
            IN + IN_ADDR.tariff + '*(1+' + IN + IN_ADDR.tariffEscal + '/100)^' + c + yrRow, '#,##0.0');
          putF(ws, c + revRow, c + genRow + '*' + c + priceRow + '/1000', FMT_M, { bold: true });
        }
      }
      putF(ws, 'D' + revRow, sumFormula(revRow), FMT_M, { bold: true });
      r = revRow + 2;

      if (anyOverride) {
        ws.getCell('B' + r).value = '※ 실측 오버라이드가 적용된 분기는 "판매단가"가 그 분기의 실제 정산단가(입력값 시트의 실측 표)이고, 영업수익은 발전량 × 판매단가 / 1000 으로 계산합니다(값은 원본과 그대로 일치). 이 분기의 트랙별 매출 행은 A/R(회수 시차) 계산용 배분치입니다.';
        ws.getCell('B' + r).font = { name: FONT, size: 8, italic: true, color: { argb: 'FF9AA6A1' } };
        r += 2;
      }
      ...
      /* =========================================================
         ...
         트랙별 매출은 위에서 이미 만든 행을 그대로 쓴다(재계산하지 않음).
         ========================================================= */
      ...
      if (arLagAny) {
        ...
        section(ws, r, '매출채권(A/R) 회수 시차'); r += 2;
        var arBalRow = {};
        var trackIdx = hasTracks ? IN_ADDR.tariffTrack.map(function (_, i) { return i; }) : [0];
        trackIdx.forEach(function (ti) {
          var nm = hasTracks ? trackName(ti) : '전체';
          var srcRow = hasTracks ? trackRevRow[ti] : revRow;
          arBalRow[ti] = r;
        ...
        AR_WC_ROW = r;
        label(ws, r, '운전자본 증감(A/R)', '[KRWm]', { bold: true, fill: SUB_FILL });
        for (var n = 0; n < N; n++) {
          var terms = trackIdx.map(function (ti) {
            var prev = n > 0 ? (pc(n - 1) + arBalRow[ti]) : '0';
            return prev + '-' + pc(n) + arBalRow[ti];
          }).join('+');
          putF(ws, pc(n) + r, terms, FMT_M, { bold: true });
        }
        putF(ws, 'D' + r, sumFormula(r), FMT_M, { bold: true });
        r++;
        ws.getCell('B' + r).value = '※ 20년 전체 합계는 0입니다(초기에 묶인 미수금을 마지막에 전액 회수). 다만 분기별로는 크게 움직여서 준공 직후 DSCR과 Equity IRR에 영향을 줍니다.';
        ws.getCell('B' + r).font = { name: FONT, size: 8, italic: true, color: { argb: 'FF9AA6A1' } };
      }
    })();
    ...
    (function () {
      ...
      for (var n = 0; n < N; n++) putF(ws, pc(n) + r, "'Revenue'!" + pc(n) + REV_TOTAL_ROW, FMT_M, { bold: true });
      putF(ws, 'D' + r, sumFormula(r), FMT_M, { bold: true });
      r += 2;
    ...
    (function () {
      ...
      for (var n = 0; n < N; n++) {
        putF(ws, pc(n) + 9,
          "'Revenue'!" + pc(n) + REV_TOTAL_ROW + '-\'Opex\'!' + pc(n) + OPEX_SENIOR_ROW +
      ...
      for (var n = 0; n < N; n++) {
        var divF;
        if (n === lastOpIdx2) {
          // 청산배당: 한도 없이 잔여현금 전액. 단 현금이 음수면 0 —
          // 엔진(Math.max(0, avail))과 같게. 예전엔 음수 현금이 그대로
          // '음수 배당'으로 나가 투자자 IRR 이 망가졌다.
          divF = 'MAX(0,' + pc(n) + availRow + ')';
        } else if (hasDivMonth) {
      ...
      for (var n = 0; n < N; n++) {
        ...
        putF(ws, pc(n) + r,
          "'Revenue'!" + pc(n) + REV_TOTAL_ROW + "-'Opex'!" + pc(n) + OPEX_TOTAL_ROW +
      ...
      CFQ_EQIRRPRE_ROW = irrRow('Equity IRR(FCFE 세전)', CFQ_EQFLOWPRE_ROW, model.kpi.equityIRRPre || 0.12);
      CFQ_EQIRR_ROW = irrRow('Equity IRR(FCFE 세후)', CFQ_EQFLOW_ROW, model.kpi.equityIRR || 0.1);
    ...
    (function () {
      ...
      // 화면(HTML) 결과 카드와 같은 이름·같은 순서. E열에 한 줄 설명.
      function irrKv(name, row, note) {
        kvF(name, "'CF(Q)'!D" + row, FMT_P);
        ws.getCell('E' + (r - 1)).value = note;
        ws.getCell('E' + (r - 1)).font = { name: FONT, size: 8, color: { argb: 'FF6B7B76' } };
      }
      irrKv('Equity IRR (배당) [%]', CFQ_DIVIRR_ROW, '출자자가 실제로 받는 배당 기준(세후) — 가장 보수적인 대표 수익률');
    ...
    /* =========================================================
       입력값 참조 정리 — "기본값은 입력값에서, 계산은 그 시트 안에서".
       예전에는 분기 계산 수식마다 '입력값'!C7·C14·C12… 를 직접 섞어 써서,
       예를 들어 Revenue 의 발전량 한 칸에 입력값 참조가 4~5개씩 들어갔다.
       이제 각 계산 시트 맨 아래 "가정 (입력값 연결)" 블록에서 입력값을
       딱 한 번씩만 끌어오고, 위의 분기 계산은 그 셀($D$n)만 참조한다.
         - 한 값(설비용량·금리 등) → 블록의 한 칸(D열), 계산은 $D$n 절대참조
         - 분기마다 다른 값(공사비 지출 비중·실측치 등) → 블록의 한 행,
           계산은 같은 열의 그 행을 참조
         - 이미 "='입력값'!C7" 처럼 입력값을 그대로 가져오기만 하는 셀은
           그 자체가 연결 셀이므로 손대지 않는다.
       행을 끼워 넣지 않고 맨 아래에 붙이므로 다른 시트가 참조하는 행
       번호는 전혀 바뀌지 않는다.
       ========================================================= */
    (function localizeInputs() {
      var inWs = wb.getWorksheet('입력값');
      if (!inWs) return;
      var TOK = /'입력값'!\$?([A-Z]{1,3})\$?(\d+)/g;
      var RANGE = /'입력값'!\$?[A-Z]{1,3}\$?\d+:/;
      var PURE = /^'입력값'!\$?[A-Z]{1,3}\$?\d+(\*100|\/100)?$/;
      var LINK_COLOR = 'FF1E7B34';   // 재무모델 관례: 초록 = 다른 시트 연결

      function inLabel(col, row) {
        var b = inWs.getCell('B' + row).value;
        var base = (b === null || b === undefined) ? ('입력값 ' + col + row) : String(b).trim();
        // 표(트랜치·트랙·항목 등) 안의 셀이면 그 열의 머리글을 붙인다
        // (예: 선순위A · 운영금리[%], SMP · 비중[%]). 표 머리행은 B열이
        // 아래 이름 중 하나인 행이다 — 일반 키-값 행은 머리글이 없다.
        var TABLE_HEADS = { '트랜치': 1, '트랙': 1, '항목': 1, '구  분': 1, '사업자': 1, '연도': 1 };
        for (var hr = row - 1; hr >= Math.max(1, row - 20); hr--) {
          var hb = inWs.getCell('B' + hr).value;
          if (typeof hb === 'string' && TABLE_HEADS[hb.trim()]) {
            var hv = inWs.getCell(col + hr).value;
            return (typeof hv === 'string' && hv.trim()) ? (base + ' · ' + hv.trim()) : base;
          }
          if (typeof hb === 'string' && inWs.getCell('C' + hr).value === null &&
              inWs.getCell('B' + hr).fill && inWs.getCell('B' + hr).fill.fgColor &&
              inWs.getCell('B' + hr).fill.fgColor.argb === HDR_FILL) break;   // 섹션 제목을 넘어가면 중단
        }
        return col === 'C' ? base : (base + ' · ' + col + '열');
      }

      ['Funding', 'Debt', 'Revenue', 'Opex', 'IS(Q)', 'CF(Q)'].forEach(function (sn) {
        var ws = wb.getWorksheet(sn);
        if (!ws) return;
        var targets = [], lastRow = 0;
        ws.eachRow({ includeEmpty: false }, function (row, rn) {
          ...
          row.eachCell({ includeEmpty: false }, function (cell) {
            ...
            if (v.formula.indexOf("'입력값'!") < 0) return;
            if (PURE.test(v.formula)) return;           // 이미 순수 연결 셀
            if (RANGE.test(v.formula)) return;          // 범위 참조는 그대로(현재 없음)
            targets.push(cell);
          });
        });
        if (!targets.length) return;

        // 1) 입력값 행별로 "분기마다 다른 값인지(같은 열을 따라감)" 판정
        var byRow = {};
        targets.forEach(function (cell) {
          var f = cell.value.formula, m;
          TOK.lastIndex = 0;
          while ((m = TOK.exec(f))) {
            var e = byRow[m[2]] || (byRow[m[2]] = { follows: true, cols: {}, refs: {} });
            e.refs[m[1]] = true;
            e.cols[cell.col] = true;
            if (colLetter(cell.col) !== m[1]) e.follows = false;
          }
        });

        // 2) 블록 작성 — 기존 내용 아래에 붙인다
        var r = lastRow + 3;
        section(ws, r, '가정 (입력값 연결) — 이 시트의 계산이 쓰는 원천값');
        ws.getCell('D' + r).value = '값';
        ws.getCell('D' + r).font = { name: FONT, bold: true, size: 9, color: { argb: WHITE } };
        r += 1;
        var note = ws.getCell('B' + r);
        note.value = '※ 입력값 시트는 여기서만 참조합니다(초록 글자 = 입력값 연결). 위의 분기 계산은 이 셀들만 참조하므로, 값을 바꾸려면 입력값 시트를 고치세요.';
        note.font = { name: FONT, size: 8, italic: true, color: { argb: 'FF9AA6A1' } };
        r += 1;

        var scalarAddr = {}, rowAddr = {};
        Object.keys(byRow).map(Number).sort(function (a, b) { return a - b; }).forEach(function (inRow) {
          var e = byRow[inRow];
          var perColumn = e.follows && Object.keys(e.cols).length > 1;
          if (perColumn) {
            // 분기별 행(지출 비중·실측치 등)은 행 이름만 쓴다 — 열 머리글은 날짜라 붙이지 않는다.
            var bl = inWs.getCell('B' + inRow).value;
            label(ws, r, (bl === null || bl === undefined) ? ('입력값 ' + inRow + '행') : String(bl).trim(), null, { indent: true });
            ws.getCell('C' + r).value = '입력값 ' + inRow + '행';
            ws.getCell('C' + r).font = { name: FONT, size: 8, color: { argb: 'FF9AA6A1' } };
            Object.keys(e.cols).map(Number).forEach(function (col) {
              var L = colLetter(col);
              var src = inWs.getCell(L + inRow);
              putF(ws, L + r, IN + L + inRow, src.numFmt || FMT_M, { color: LINK_COLOR });
            });
            rowAddr[inRow] = r;
            r++;
          } else {
            Object.keys(e.refs).sort().forEach(function (L) {
              label(ws, r, inLabel(L, inRow), null, { indent: true });
              ws.getCell('C' + r).value = L + inRow;
              ws.getCell('C' + r).font = { name: FONT, size: 8, color: { argb: 'FF9AA6A1' } };
              var src = inWs.getCell(L + inRow);
              putF(ws, 'D' + r, IN + L + inRow, src.numFmt || FMT_M, { color: LINK_COLOR });
              scalarAddr[L + inRow] = '$D$' + r;
              r++;
            });
          }
        });

        // 3) 계산 수식의 입력값 참조를 블록 셀 참조로 바꾼다
        targets.forEach(function (cell) {
          var f = cell.value.formula.replace(TOK, function (all, L, row) {
            if (rowAddr[row] != null) return colLetter(cell.col) + rowAddr[row];
            return scalarAddr[L + row] || all;
          });
