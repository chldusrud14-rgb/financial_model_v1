(function () {
  ...
  function loadDangjin() {
    ...
    // 소수 둘째 자리로 자르지 않는다 — 아래에서 켜는 총사업비 항목 합계(=TIC_exIDC)와
    // 정확히 같아야 총사업비를 다시 쓰지 않고, 그래야 지출 스케줄 재생성이 일어나지 않는다.
    setVal('[data-k="capexEok"]', +(ref.funding.TIC_exIDC / 100).toFixed(6));
    ...
    if (ref.capexItems && ref.capexItems.length) {
      CAPEX_ITEMS = ref.capexItems.map(function (it) { return { name: it.name, amountEok: +(it.amountKRWm / 100).toFixed(6) }; });
